...
namespace Klient1TabulkaDB
{
    ...
    public class EditBox : Control
    {
        ...
        object val
        {
            get
            {
                if (editControl is DatePicker)
                    return (editControl as DatePicker).Text;
                else
            ...
            {
                if (editControl is DatePicker)
                    (editControl as DatePicker).Text = value as string;
                else
                    (editControl as TextBox).Text = value as string;
            }
        }
        object oldValue;

        public static readonly DependencyProperty EditTypeProperty = DependencyProperty.Register("EditType", typeof(AttributeType), typeof(EditBox), new FrameworkPropertyMetadata(null));

        /// <summary>
        /// Gets or sets the type of the EditBox
        /// </summary>
        public AttributeType EditType
        {
            get { return (AttributeType)GetValue(EditTypeProperty); }
        ...
        public bool IsEditing
        {
            ...
            private set
            {
                if (val == null | (val as string).Length < 1)
                    val = oldValue;
                SetValue(IsEditingProperty, value);
                _adorner.UpdateVisibilty(value);
                if (ValueChanged != null & !value)
                {
                    if (oldValue != null & oldValue != val)
                    {
                        ValueChanged.Invoke(this, new ValueChangedEventArgs(oldValue, val));
                        oldValue = val;
                    }
                }
            }
        ...
        private void OnTextBoxKeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (IsEditing & e.Key == Key.Escape)
            {
                val = oldValue;
                IsEditing = false;
                _canBeEdit = false;
        ...
        {
            Debug.WriteLine("LostFocus = true");
            if ((editControl is DatePicker))
            {
                if (!(editControl as DatePicker).IsDropDownOpen & IsEditing)
                {
                    IsEditing = false;
                }
            }
        ...
        private FrameworkElement editControl;
